...
describe("Lending Pool Tests", () => {
  ...
  it("Borrower can borrow, gets liquidated if price drops below liquidation threshold", { timeout: 180000 }, () => {
    const initialLiquidatorBalance = getSBTCBalance(liquidator);
    ...
    expect(getSBTCBalance(liquidator)).toBe(initialLiquidatorBalance);
    ...
    expect(getSBTCBalance(liquidator)).toBe(initialLiquidatorBalance + 100);
